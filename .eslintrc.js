...
module.exports = {
  ...
  rules: {
    ...
    // 'valid-jsdoc': 1, // （建议）强制使用有效的 JSDoc 注释
    ...
    //   ignore: [-1, 0, 1, 2],
    // }], // (允许)   禁止使用魔术数字，魔术数字是在代码中多次出现的没有明确含义的数字。它最好由命名常量取代。
    ...
    // 'no-return-assign': 0, // （允许，es6 中太常见了）禁止在返回语句中赋值
    ...
    // 'no-useless-escape': 1, // 建议禁用不必要的转义
    ...
    // 'array-element-newline': [1, { multiline: true }], // ??? 建议数组元素间出现换行
    // 'block-spacing': 2, // 强制在代码块中开括号前和闭括号后有空格
    // 'brace-style': 2, // 大括号风格要求
    // 'camelcase': 2, // 要求使用骆驼拼写法
    // 'comma-dangle': [2, 'never'], // （禁止使用拖尾逗号）要求或禁止使用拖尾逗号
    ...
    // 'func-style': [1, 'declaration'], // ??? (建议)强制 function 声明或表达式的一致性
    ...
    // 'space-before-function-paren': [1, 'always'], // 禁止函数圆括号之前有一个空格
    ...
    // 'object-shorthand': 1, // (建议)要求对象字面量简写语法
    // 'prefer-arrow-callback': 2, // 要求使用箭头函数作为回调
    // 'prefer-const': 2, // ??? 要求使用const
